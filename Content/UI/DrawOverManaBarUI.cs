		...
		if (Main.netMode != NetmodeID.Server)
		{
			...
			if (player.ManaPlayer().manabank)
			{
				...
				for (int i = 0; i < l; i++)
				{
					...
					Main.spriteBatch.Draw(Textures.ManaBankOverlay.Value,
						new Vector2(Main.screenWidth - 25 - 16 + 6, 37 + 16 + (i * 7)), new Rectangle(24, 0, 12, 8), c,
						0f, Textures.GenericItem.Size() / 2f, 1f, SpriteEffects.None, 0);
				...
				Main.spriteBatch.Draw(Textures.ManaBankOverlay.Value,
					...
					Textures.GenericItem.Size() / 2f, 1f, SpriteEffects.None, 0);
				Main.spriteBatch.Draw(Textures.ManaBankOverlay.Value, new Vector2(Main.screenWidth - 25 - 16, 37),
					new Rectangle(0, 0, 22, 30), Color.White, 0f, Textures.GenericItem.Size() / 2f, 1f,
		...
		if (Main.netMode != NetmodeID.Server)
		{
			...
			if (player.ManaPlayer().manaToRegen > 0) {
				for (int i = 0; i < m + 1; i++)
				{
					...
					Main.spriteBatch.Draw(Textures.ManaStarGlowverlay.Value,
			...
			{
				Main.spriteBatch.Draw(Textures.Wishbone.Value, new Vector2(Main.screenWidth - 23 - 4, 42 + (m * 22)),
					null, Color.White, 0f, Textures.GenericItem.Size() / 2f, 1f, SpriteEffects.None, 0);
			}
